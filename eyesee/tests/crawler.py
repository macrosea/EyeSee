    ...
    def save_image(self, rsp_data, word):
        ...
        for image_info in rsp_data['data']:
            try:
                ...
                print("已下载" + str(self.__counter) + "张")
                self.__counter += 1
        return
...
if __name__ == '__main__':
    if len(sys.argv) > 1:
        ...
        crawler.start(args.word, args.total_page, args.start_page, args.per_page)  # 抓取关键词为 “艺术”，总数为 1 页（即总共 1*60=60 张），开始页码为 2
    else:
        # 如果不指定参数，那么程序会按照下面进行执行
        crawler = Crawler(0.05)  # 抓取延迟为 0.05

        crawler.start('艺术', 10, 2, 30)  # 抓取关键词为 “艺术”，总数为 1 页，开始页码为 2，每页30张（即总共 2*30=60 张）
